...
define.class('$base/node', function(require, exports, self){
	...
	self.atConstructor = function(obj){
		if(obj) for(var key in obj) this[key] = obj[key]
	}

	self.RAD = '1'
	self.DEG = '0.017453292519943295'
	self.PI = '3.141592653589793'
	self.PI2 = '6.283185307179586'
	self.E = '2.718281828459045'
	self.LN2 = '0.6931471805599453'
	self.LN10 = '2.302585092994046'
	self.LOG2E = '1.4426950408889634'
	self.LOG10E = '0.4342944819032518'
	self.SQRT_1_2 = '0.70710678118654757'
	self.SQRT2 = '1.4142135623730951'

	// set by the device at render start
	self.device = null
	self.extensions = ''
	...
	self.OES_standard_derivatives = 1

	self.precision = 'highp'

	self.compileHeader = function(){
	...
	self.compileAttributes = function(vtxattr, pixattr){
		var ret = ''
		var attr = {}
	...
	self.compileAttribRename = function(vtxattr, pixattr){
		var ret = ''
		if(vtxattr) for(var key in vtxattr){
			ret += '\t' + key + ' =  _' + key + ';\n'
		}
	...
	self.compileVaryings = function(varyings, name){
		var ret = ''
	...
	self.compileUniforms = function(uniforms){
		var ret = ''
	...
	self.compileUniformRename = function(uniforms){
		var ret = ''
		for(var key in uniforms){
			ret += '\t' + key + ' = _' + key + ';\n'
		}
	...
	self.compileFunctions = function(call, mask){
		var ret = ''
		var init
	...
	self.compileTextures = function(textures){
		var ret = ''
	...
	self.compileStructs = function(structs){
		var ret = ''
	...
	self.mapUniforms = function(gl, shader, uniforms, uniset, unilocs){
	...
	self.mapTextures = function(gl, shader, textures, texlocs){
		for(var key in textures){
	...
	self.mapAttributes = function(gl, shader, attributes, attrlocs, context){
	...
	self.annotateLines = function(text){
	...
	self.toVec4 = function(str, ast, str2, ast2){
		if(ast.infer === vec4){
	...
	self.decodeBlendFactor = function(node, key){
		var gl = gltypes.gl
	...
	self.decodeBlendEquation = function(eq, value){
		var gl = gltypes.gl
		...
		if(eq.type == 'Binary' && (eq.op == '+' || eq.op == '-')){ // its the main equation
			...
			out.dst = self.decodeBlendFactor(left, 'dst_color')
			out.src = self.decodeBlendFactor(right, 'src_color')
		}
			...
			if(eq.right.name == 'dst_color'){
				out.src = gl.ZERO
				out.dst = self.decodeBlendFactor(eq, 'dst_color')
			}
			else if(eq.right.name == 'src_color'){
				out.dst = gl.ZERO
				out.src = self.decodeBlendFactor(eq, 'src_color')
			}
	...
	self.decodeDepthEquation = function(gl, eq, value){
		var out = {original:value}
		if(!eq) return out
	...
	self.decodeStencilEquation = function(gl, eq, value){
		if(!eq) return {}
		else 
		if(eq.type == 'Value'){
	...
	self.alpha_blend = ''
	self.depth_test = 'src_depth > dst_depth'
	self.color_blend = '(1 - src_alpha) * dst_color + src_alpha * src_color'
	...
	self.monitorCompiledProperty = function(name, init){
		var get = '_' + name
		this[get] = init
	...
	self.getLocations = function(gl, shader, vtx_state, pix_state){
	...
	self.compileShader = function(){
		...
		var shader = this.device.shadercache[cache_id]
		...
		shader = this.device.shadercache[cache_id] = gl.createProgram()
	...
	// compile the shader
	self.compile = function(in_draw){

		if(this.device && this.dirty === false && !this.hasOwnProperty('shader')){
			...
			if(!proto.hasOwnProperty('shader')){
				this.shader = proto.shader = this.compileShader()
			}
			else{
		...
		if(in_draw) this.connectWires()
		//	console.log("HII")
	}		

	self.useShader = function(gl, shader){
	...
	self.compile_use = false

	self.useShaderTemplate = function(gl, shader, root){
		// use the shader
		gl.useProgram(shader)
	...
	self.compileUse = function(shader){
	...
	// lets draw ourselves
	self.draw = function(start, end){
		if(!this.hasOwnProperty('shader') || !this.shader) this.compile(true)
		var gl = this.device.gl
	...
	self.drawGuid = function(start, end){
		if(!this.hasOwnProperty('shader') || !this.shader) this.compile(true)
		var gl = this.device.gl
	...
	self.drawDebug = function(start, end){
		if(!this.hasOwnProperty('shader') || !this.shader) this.compile(true)
		if(!this.shader.debug) return
		var gl = this.device.gl
	...
	// draw a root node
	self.renderTo = function(nest){
	}

	self.atExtend = function(){
